	  ...
	  for (ob = &object_head; (ob != NULL); ob = ob->next)
	      ...
	      if ((ob->type == player) || (ob->type == enemy))
		{
		  ...
		  for (x = left; x < right; x++)
			...
			{
			  if ((masks[p->type][p->img_i])->bits[(int) (x - p->x)][(int) (y
			      - p->y)] == 1
			      && (masks[ob->type][ob->img_i])->bits[(int) (x
				  - ob->x)][(int) (y - ob->y)] == 1)
			    {
			      if (ob->type == enemy)
				{
...
object_draw ()
{
  ...
  for (p = &object_head; (p != NULL); p = p->next)
	...
	{
	  al_draw_bitmap (
	      sprites[p->type][p->img_i], (int) p->x, (int) p->y,
	      ((p->type == enemy) && p->vx < 0) ? ALLEGRO_FLIP_HORIZONTAL : 0);
	  ...
	  if (!strcmp (p->String, "Seeker"))
	    {
	      ...
	      dy = (pl->y - p->y) / 10;
	      p->vx +=
		  (dx > 0) ? ((dx * dx) / 300 < 0.3) ? (dx * dx) / 300 : 0.3
		  : ((dx * dx) / 300 < 0.3) ? -(dx * dx) / 300 : -0.3;
	      p->vy += (float) dy / 100;
	    }
	  if (p->vx > e_vel_max)
	    p->vx = e_vel_max;
...
int
anim (Object *object, int frame_delay, int vector_size)
{
